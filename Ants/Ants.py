...
class Place(object):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if self.ant.can_contain(insect):
                    ...
                    self.ant = insect
                elif self.ant.is_container == insect.is_container:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if isinstance(insect, QueenAnt) and insect.is_true_queen == True:
                return
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        distance, new_place = 0, self.place
        for k in range(self.min_range):
            new_place = new_place.entrance
            distance =+ 1
        while not new_place.bees and not new_place.entrance is hive and distance < self.max_range:
            new_place = new_place.entrance
            distance += 1
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.is_true_queen:
            ...
            current_spot = self.place.exit
            while current_spot:
                ant = current_spot.ant
                if ant:
                    if ant not in QueenAnt.ants_helped:
                        ant.damage *= 2
                        QueenAnt.ants_helped.append(ant)
                    if isinstance(ant, BodyguardAnt) and ant.contained_ant and ant.contained_ant not in QueenAnt.ants_helped:
                        ant.contained_ant.damage *= 2
                        QueenAnt.ants_helped.append(ant.contained_ant)
