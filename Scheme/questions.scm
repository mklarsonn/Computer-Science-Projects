...
(define (enumerate s)
  ; BEGIN PROBLEM 17
  (define (helper position s)
    (cond
      ((null? s) nil)
      (else
        (cons
          (list position (car s))
          (helper (+ 1 position) (cdr s)) )
        )))
  (helper 0 s))
...
(define (list-change total denoms)
  ; BEGIN PROBLEM 18
  (define(cons-all x lst-of-lsts)
    (if (null? lst-of-lsts) nil
        (cons (cons x (car lst-of-lsts))
              (cons-all x (cdr lst-of-lsts)))))
