...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            # BEGIN PROBLEM 2
            src.remove_front()
            first_after_dot = scheme_read(src)
            second_after_dot = read_tail(src)
            if second_after_dot is not nil:
                raise SyntaxError('There must be exactly one element following a dot')
            else:
                return first_after_dot
            # END PROBLEM 2
        else:
            # BEGIN PROBLEM 1
