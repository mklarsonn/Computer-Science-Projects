...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        big_first_operator = scheme_eval(first, env)
        check_procedure(big_first_operator)

        all_ops = rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(big_first_operator, all_ops, env)
        #return scheme_apply(scheme_eval(first), (second.map(scheme_eval)))
        # END PROBLEM 5
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('incorrect number of arguments for {0}'.format(self.fn))
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(func_name, do_lambda_form(expressions, env))
        return func_name
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        rest = expressions.second is nil
        val = scheme_eval(expressions.first, env, rest)
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        all_symbols = Pair(symbol, all_symbols)
        all_values = Pair(value, all_values)
        bindings = bindings.second

    check_formals(all_symbols)

    return env.make_child_frame(all_symbols, all_values)
