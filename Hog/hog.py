...
def is_swap(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    tens_player = (player_score % 100) // 10
    ones_player = player_score % 10
    abs_player = abs(tens_player - ones_player)

    tens_opponent = (opponent_score % 100) // 10
    ones_opponent = opponent_score % 10
    abs_opponent = abs(tens_opponent - ones_opponent)

    return abs_player == abs_opponent
    # END PROBLEM 4
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def f(score0, score1):
        ...
        if current_high < score_increase:
            current_high = score_increase
            print(score_increase, "point(s)! That's the biggest gain yet for Player", who)
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    make_avg_roll = make_averaged(roll_dice)
    count = 1
    high_avg_score = 0
    die_num = 0
    while count <= 10:
        new_score = make_avg_roll(count, dice)
        if new_score > high_avg_score:
            high_avg_score = new_score
            die_num = count
        count += 1
    return die_num
    # END PROBLEM 9
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 11
    hypothetical_score = score + free_bacon(opponent_score)
    if is_swap(hypothetical_score, opponent_score):
        if hypothetical_score < opponent_score:
            return 0

    elif free_bacon(opponent_score) >= margin:
        if not(is_swap(hypothetical_score, opponent_score)):
            return 0
