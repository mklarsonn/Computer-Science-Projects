...
def group_by_centroid(restaurants, centroids):
    ...
    for k in restaurants:
        closest_centroid = find_closest(restaurant_location(k), centroids)
        list_centroids += [[closest_centroid, k]]
    return group_by_first(list_centroids)
    # END Question 4
...
def find_predictor(user, restaurants, feature_fn):
    ...
    # BEGIN Question 7
    xList = [x - mean(xs) for x in xs]
    sumX = sum([pow(arg,2) for arg in xList])


    yList = [y - mean(ys) for y in ys]
    sumY = sum([pow(arg,2) for arg in yList])


    xy = zip(xList, yList)

    sum_xy = sum( i[0] * i[1] for i in xy )
    b= sum_xy/sumX
    a=mean(ys) - b*mean(xs)
    r_squared =pow(sum_xy,2)/(sumX*sumY)
    # END Question 7
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for fn in feature_fns:
        fn, new_r = find_predictor(user, reviewed, fn)
        if new_r > highest:
            highest = new_r
            lst = [fn]
    return lst[0]
